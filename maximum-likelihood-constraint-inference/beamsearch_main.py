...
# Adding necessary argument parsing
...
# Setup parameters for the environment
...
# Attempt to load the trajectory data
try:
...
# For showing new demonstrations, if required
...
# Placeholder for the action plot function
def action_plot(grid, obstacles, iternum):
    ...
    plt.xlabel(f"{iternum}")
    plt.savefig(f"figures/{iternum}.png")

# Reward, Success, and Violation tracking initialization
reward_history = []  # <-- New List for tracking average reward
success_rate_history = []  # <-- New List for tracking success rate
violation_rate_history = []  # <-- New List for tracking violation rate

# Main Loop for Constraint Inference and Beam Search Integration
if args.do_constraint_inference:
    ...
    # Perform initial backward and forward pass
    ...
    for i in range(n_constraints):
        ...
        # Beam Search Experiment - Tracking metrics for reward, success rate, and violation rate
        reward_history.append(current_reward)  # <-- Tracking Average Reward
        current_sr = estimated_mdp.get_success_rate()
        success_rate_history.append(current_sr)  # <-- Tracking Success Rate
        print(success_rate_history)
        current_vr = estimated_mdp.get_violation_rate()
        violation_rate_history.append(current_vr)  # <-- Tracking Violation Rate
...
# Save the success_rate_history list to a CSV file
...
# Save the violation_rate_history list to a CSV file
